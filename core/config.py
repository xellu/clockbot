    ...
    def load(self):
        ...
        try:
            ...
            if self.fault_retry_count >= self.fault_retry_treshold:
                EventBus.signal("warn", "Core.Config", f"Unable to load configuration file, automatic overwrite aborted (max retries exceeded)")
                return

            EventBus.signal("error", Exception("Failed to load"), "Core.Config", f"Unable to load configuration file, automatic overwrite aborted (max retries exceeded)", fatal=True)
